...
export const Tasks = ({className=""}) => {
  ...
  function handleDark(){
    setDark(prev => {
      ...
      html.className = nextdark ? "dark" : "";
      return nextdark;
    })}

  const { isVisible, setVisible } = useContext(VisibleContext);
  ...
  {task: `${dark ? 'Dark' : 'Light'} Theme` , icon: dark ? (<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  ...
  , onclick: (handleDark)}
    ...
    return <AnimatePresence mode="wait">
      {isVisible ? (<motion.div transition={{ ease: easeInOut, duration: 0.3}} key="tasks" initial={{ opacity: 1, x: '-100%' }}
                        animate={{ opacity: 1, x: 0}}
                        exit={{ opacity: 0, x: '-100%' }} className={`${className}`}>
        <Card className={`${className}`}>
          <button onClick={() => setVisible(e => !e)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
            </svg>
          </button>
          {Taskcomponent()}
        </Card>
      </motion.div>) : (<button className={`${isVisible ? "hidden" : "visible"} z-10 mb-auto`} onClick={() => setVisible(e => !e)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6 stroke-gray-400">
